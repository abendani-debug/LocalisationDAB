...
- **Verrou week-end** (`Signalement.computeExpiresAt`) : tout signalement (**admin ou utilisateur**) avec état `vide` ou `en_panne`, créé le **jeudi à partir de 16h (heure Algérie, UTC+1)**, reste valide jusqu'au **dimanche 7h** au lieu de la durée habituelle (4h utilisateur / 24h admin) — un DAB vide/en panne le jeudi soir a de fortes chances de le rester tout le week-end (pas de réapprovisionnement/réparation avant dimanche). Un signalement `disponible` garde toujours sa durée normale, admin comme utilisateur (un DAB plein le jeudi peut se vider avec l'usage réel pendant le week-end, notamment en zone commerciale à fort trafic). Le reste de la semaine n'est pas concerné. Si un nouveau signalement arrive pendant cette fenêtre, il recalcule `etat_communautaire` normalement — le verrou n'empêche pas les mises à jour, il ne fait qu'étendre la durée par défaut en absence de nouveau signalement.
